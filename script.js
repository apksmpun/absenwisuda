...
document.addEventListener("DOMContentLoaded", () => {
    const loadingIndicator = document.getElementById("loadingNames");
    loadingIndicator.textContent = "Mengambil data nama siswa...";
    ...
    fetch(scriptURL)
      ...
      .then(data => {
        namaSiswa = data;
        loadingIndicator.textContent = "Nama siswa berhasil dimuat.";
        setTimeout(() => loadingIndicator.classList.add("hidden"), 1000);
      })
      .catch(err => {
        console.error("Gagal ambil nama:", err);
        loadingIndicator.textContent = "Gagal memuat nama siswa.";
      });
  });
